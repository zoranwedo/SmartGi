...
Partial Class Invoices_InvoiceForm
    ...
    Protected AccPeriodOpen As Boolean = False         ' AccPeriod < 1
    ...
    Protected CanCharge As Boolean = True
    ...
    Protected Function RateModal(ByVal pRateType As Integer, ByVal pRateCode As String, ByVal pBillID As Integer, ByVal pDetID As Integer) As String
        Dim res As String = pRateCode   'String.Empty

        If pRateCode <> "COMPLIMENTARY" Then
            If pRateType = 9 And CanEdit Then
                res = String.Format("ShowSuppModalRO({0});", pDetID)
            ElseIf pRateType = 9 And Not CanEdit Then
                res = String.Format("ShowSuppModalRO({0});", pDetID)
            ElseIf pRateType <> 9 And CanEdit Then
                res = String.Format("ShowStayModal({0});", pBillID)
            ElseIf pRateType <> 9 And Not CanEdit Then
                res = String.Format("ShowStayModalRO({0});", pBillID)
            End If
        End If
    ...
    Protected Sub LoadStateVariables()
        ...
        AccPeriodOpen = ViewState("AccPeriodOpen")
        ...
        CanEdit = ((IsHotelUser And IsHotelState) Or (IsCCIUser And IsCCIState And Not cbHotel.Checked And Not IsCCIReviewed And Not IsApplied And Not IsDelivered)) 'And AccPeriodOpen
        'If My.User.Name = "zoran" Then CanEdit = True
        CanCharge = (My.User.IsInRole("invoicing_SH_operator") Or My.User.IsInRole("admin")) And CanEdit
    End Sub
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If s Is Nothing Then s = "SH"
            If s <> "SH" Then
                Response.Write("<h3>This is not an SH invoice</h3>")
                Response.Write(String.Format("<h3><a href='InvoiceFormSX.aspx?InvoiceID={0}'>Go to invoice</a> or go back.</h3>", Request.QueryString("InvoiceID")))
                Response.End()
            End If
        End If
    End Sub
    ...
    'Recalculate invoice
    Public ReadOnly Property ReclaculateVisible() As Boolean
        Get
            Return CanEdit And IsAdmin And IsRecalcEnabled
        End Get
    End Property

    'fvInvoice.ItemTemplate.cbRVHtl
    'Reviewed by Hotel
    Public ReadOnly Property HtlReviewVisible() As Boolean
        Get
            Return IsHotelState And IsHotelUser
        End Get
    End Property

    'fvInvoice.ItemTemplate.hlSend
    'Sending list (to CCI)
    Public ReadOnly Property SendingVisible() As Boolean
        Get
            Return InvState = 2 And IsHotelUser
        End Get
    End Property

    'fvInvoice.ItemTemplate.cbRVCci
    'Reviewed by CCI
    Public ReadOnly Property CciReviewVisible() As Boolean
        Get
            Return IsCCIState
        End Get
    End Property

    Public ReadOnly Property CciReviewEnabled() As Boolean
        Get
            'Return IsCCIState And IsCCIUser And Not cbHotel.Checked And ((IsCCIReviewed And AccPeriodOpen And Not IsDelivered) Or Not IsCCIReviewed)
            If IsCCIReviewed Then
                Return IsCCIState And IsAdmin And Not cbHotel.Checked And Not IsDelivered And Not IsApplied
                'Return False
            Else
                Return IsCCIState And IsCCIUser And Not cbHotel.Checked And Not IsDelivered
            End If
        End Get
    End Property
    ...
    'Reset Delivery
    Public ReadOnly Property DeliverVisible() As Boolean
        Get
            Return Not IsDelivered And IsCCIReviewed And IsCCIUser And Not cbHotel.Checked And (InvState = 5 Or InvState = 6)
        End Get
    End Property

    'fvInvoice.ItemTemplate.btnUnDeliv
    'Reset Delivery
    Public ReadOnly Property UnDeliverVisible() As Boolean
        Get
            'Return IsAdmin And IsDelivered And Not cbHotel.Checked
            Return IsDelivered And Not IsCCIReviewed And IsAdmin And Not cbHotel.Checked And (InvState = 5 Or InvState = 6)
        End Get
    End Property

    'fvInvoice.ItemTemplate.btnCancel
    'Cancel invoice
    Public ReadOnly Property CancelVisible() As Boolean
        Get
            'Return IsUserInState And AccPeriodOpen And Not IsDelivered And Not IsApplied
            Return IsUserInState And Not IsDelivered And Not IsApplied
        End Get
    End Property

    'fvInvoice.ItemTemplate.btnChangeHotel
    'Change Hotel
    Public ReadOnly Property ChangeHotelVisible() As Boolean
        Get
            Return IsHotelState And IsHotelUser
        End Get
    End Property

    'fvInvoice.ItemTemplate.btnUpdate
    'Edit invoice
    Public ReadOnly Property EditInvVisible() As Boolean
        Get
            Return IsUserInState And Not IsDelivered
        End Get
    End Property

    'fvInvoice.EditItemTemplate.txtDate
    'Edit Invoice Date
    Public ReadOnly Property EditInvDateEnable() As Boolean
        Get
            'Return IsUserInState And AccPeriodOpen
            Return IsUserInState
        End Get
    End Property

    'dlStay.SelectedItemTemplate.ibEdit
    'Edit stay detail
    'Visible='<%# CanEdit %>'

    'dlStay.SelectedItemTemplate.ibDelete
    'Delete stay detail
    'Visible='<%# CanEdit %>'

    'dlStay.SelectedItemTemplate.btnStayRecalc
    'Recalc stay detail
    Public ReadOnly Property RecalcStayVisible() As Boolean
        Get
            Return CanEdit And IsAdmin And IsRecalcEnabled
        End Get
    End Property

    'dlStay.SelectedItemTemplate.dlBill.ItemTemplate.lnkEditRate
    'Functionality is determined by RateModal() and depends on CanEdit

    'dlStay.SelectedItemTemplate.dlBill.ItemTemplate.ibBillDelete
    'Delete supplement
    'Visible='<%# Eval("RateType")=9 AND CanEdit %>'

    'dlStay.SelectedItemTemplate.dlBill.ItemTemplate.ibSplit
    'Split stay detail
    'Visible='<%# Eval("RateType")<>9 AND CanEdit %>'

    'dlStay.SelectedItemTemplate.btnNewBill
    'Add supplement
    'Visible='<%# CanEdit %>'

    'dlStay.FooterTemplate.btCM
    'Add CM supplement

    'dlStay.FooterTemplate.gvCM.btnDeleteCM
    'Delete CM supplement
    ...
    Protected Sub fvInvoiceHead_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles fvInvoiceHead.DataBound
        ...
        With fvInvoiceHead
            ...
            hf = CType(.FindControl("hfAccPeriod"), HiddenField)
            If hf.Value IsNot Nothing Then AccPeriodOpen = hf.Value < 1
        ...
        'CanEdit = (IsHotelUser And IsHotelState) Or (IsCCIUser And IsCCIState And Not cbHotel.Checked And Not IsCCIReviewed And Not IsApplied And Not IsDelivered And AccPeriodOpen)
        CanEdit = True
        CanCharge = (My.User.IsInRole("invoicing_SX_operator") Or My.User.IsInRole("admin")) And CanEdit
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim gross As Boolean = CType(fvInvoiceHead.FindControl("cbGrossBill"), CheckBox).Checked
    ...
    Protected Sub dlBill_EditCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        Dim BillID As Integer = e.CommandArgument
        Dim dlBill As DataList = dlStay.SelectedItem.FindControl("dlBill")
        Dim dlBillEdit As DataList = dlStay.SelectedItem.FindControl("dlBillEdit")

        Dim index = -1

        dlBillEdit.DataBind()
        For Each i As DataListItem In dlBillEdit.Items
            Dim hfBBillingID As HiddenField = i.FindControl("hfBBillingID")
            If hfBBillingID.Value = BillID Then index = i.ItemIndex
        Next

        If index > -1 Then
            dlBill.Visible = False
            dlBillEdit.Visible = True
            dlBillEdit.EditItemIndex = index
            dlBillEdit.DataBind()

            Dim txtStayRate As TextBox = dlBillEdit.Items(index).FindControl("txtStayRate")
            txtStayRate.Text += "*"
        End If
    End Sub

    Protected Sub dlBillEdit_EditCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs)
        Dim dlBill As DataList = source
        dlBill.EditItemIndex = e.Item.ItemIndex
        dlBill.DataBind()
    End Sub

    Protected Sub dlBillEdit_CancelCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs)
        Dim dlBill As DataList = dlStay.SelectedItem.FindControl("dlBill")
        Dim dlBillEdit As DataList = dlStay.SelectedItem.FindControl("dlBillEdit")

        dlBill.Visible = True
        dlBillEdit.Visible = False
    End Sub

    Protected Sub dlBillEdit_UpdateCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs)
        Dim dlBill As DataList = dlStay.SelectedItem.FindControl("dlBill")
        Dim dlBillEdit As DataList = dlStay.SelectedItem.FindControl("dlBillEdit")
        ...
        Try
            Dim txtStayRate As TextBox = e.Item.FindControl("txtStayRate")
            Dim txtBillAmount As TextBox = e.Item.FindControl("txtBillAmount")

            Dim billingID As Integer = e.CommandArgument
            Dim rateCode As String = txtStayRate.Text
            Dim amount As Decimal = Decimal.Parse(txtBillAmount.Text)
            Dim user As String = My.User.Name

            Dim q As New dsInvoiceSXTableAdapters.QueriesTableAdapter
            q.InvBillUpdateSH(billingID, rateCode, amount, user)

            dlBillEdit.EditItemIndex = -1
            dlBillEdit.DataBind()
            dlBill.Visible = True
            dlBillEdit.Visible = False
            RefreshData()
...
#Region "Concept"
    Protected Sub dlConcept_EditCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs)
        Dim dlConcept As DataList = source
        dlConcept.EditItemIndex = e.Item.ItemIndex
        dlConcept.DataBind()
    End Sub

    Protected Sub dlConcept_CancelCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs)
        Dim dlConcept As DataList = source
        dlConcept.EditItemIndex = -1
        dlConcept.DataBind()
    End Sub

    Protected Sub dlConcept_UpdateCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs)
        Dim dlConcept As DataList = source

        Dim invoiceID As Integer = Integer.Parse(Request.QueryString("InvoiceID"))
        Dim detailID As Integer = CType(e.Item.FindControl("hfcDetailId"), HiddenField).Value
        Dim conceptId As Integer = CType(e.Item.FindControl("hfcConceptId"), HiddenField).Value
        Dim amount As Decimal = CType(e.Item.FindControl("txtcAmount"), TextBox).Text

        Dim q As New dsConceptTableAdapters.InvStayConceptTableAdapter
        Try
            q.Update(invoiceID, detailID, conceptId, amount, invoiceID, detailID, conceptId)
            dlConcept.EditItemIndex = -1
            dlConcept.DataBind()
        Catch ex As Exception
            popShow("Error Updating concept", ex)
        End Try
    End Sub

    Protected Sub dlConcept_DeleteCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs)
        Dim dlConcept As DataList = source
        ...
        Dim q As New dsConceptTableAdapters.InvStayConceptTableAdapter
        Try
            q.Delete(invoiceID, detailID, conceptId)
            dlConcept.EditItemIndex = -1
            dlConcept.DataBind()
        Catch ex As Exception
            popShow("Error Deleting concept", ex)
        End Try
    End Sub


    Protected Sub dumyConcept_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles dumyConceptRefresh.Click
        Dim dlConcept As DataList = dlStay.SelectedItem.FindControl("dlConcept")
        dlConcept.DataBind()
    End Sub
