...
Partial Class Reports_Contracts_TourOpReport
    ...
    Protected Sub FilterData()
        ...
        If ctrlCreated.HasFromValue Then
            Dim fDate As Date = Date.Parse(ctrlCreated.DateFrom)
            Dim sfDate As String = String.Format("'{0:yyyy-MM-dd}'", fDate)
            f.Add("Created >= " & sfDate)
        End If

        If ctrlCreated.HasToValue Then
            Dim tDate As Date = Date.Parse(ctrlCreated.DateTo)
            Dim stDate As String = String.Format("'{0:yyyy-MM-dd}'", tDate)
            f.Add("Created <= " & stDate)
        End If

        If f.Count = 2 Then
            dsTOSearch.FilterExpression = String.Format("{0} AND {1}", f(0), f(1))
        ElseIf f.Count = 1 Then
            dsTOSearch.FilterExpression = String.Format("{0}", f(0))
        Else
            dsTOSearch.FilterExpression = Nothing
        End If
    End Sub
End Class
