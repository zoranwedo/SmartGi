...
Partial Class Reports_Invoices_InvoiceJournal_TO
    ...
    Protected Sub RenderReport()
        If rbCompare.Checked Then
            CompareReport()
        ElseIf rbDetail.Checked Then
            DetailReportNew()
        Else
            StandardReport()
        End If
    ...
    Protected Sub StandardReport()
        Dim tStart = Now
        Me.LogReport(LogApp.stateStart, "Report rendering started")

        'Report
        Dim lr As New Microsoft.Reporting.WebForms.LocalReport
        If rbPlain.Checked Then
            lr.ReportPath = "Reports\Invoices\RptInvJournal.rdlc"
        ElseIf rbTTOO.Checked Then
            lr.ReportPath = "Reports\Invoices\RptInvJournal_TO.rdlc"
        Else
            lr.ReportPath = "Reports\Invoices\RptInvJournal_TO_GR.rdlc"
        End If
        ...
        Dim pTOgroup As New ReportParameter("TOgroup", True)                    'Obsolete control and parameter
        ...
        Dim pdateTo As ReportParameter = IIf(ctrlCheckIn.HasToValue, New ReportParameter("dateTo", ctrlCheckIn.DateTo), New ReportParameter("dateTo"))
        Dim pCM As New ReportParameter("CM", cbCM.Checked)
        Dim p As ReportParameter() = {pdateFrom, pdateTo, puser, photel, pfilter, pTOgroup, pHotelName, pCM}
        lr.SetParameters(p)

        lr.DataSources.Clear()
        lr.DataSources.Add(New Microsoft.Reporting.WebForms.ReportDataSource("dsCoopMarketing_InvoiceJournal", dsFilteredInvoices))
        FilterString()
    ...
    Protected Sub CompareReport()
        ...
        'Report
        Dim lr As New Microsoft.Reporting.WebForms.LocalReport
        lr.ReportPath = "Reports\Invoices\RptInvoiceJournalCompConcept.rdlc"
        'If cbHotel.Checked Then
        '    lr.ReportPath = "Reports\Invoices\RptInvoiceJournalCompRef.rdlc"
        'Else
        '    lr.ReportPath = "Reports\Invoices\RptInvoiceJournalCompConcept.rdlc"
        'End If
        lr.DataSources.Clear()
        lr.DataSources.Add(New ReportDataSource("dsConcept_Invoice_Prev_GRP_Concept", dsFilteredInvoicesCompConcept))
        'If cbHotel.Checked Then
        '    lr.DataSources.Add(New ReportDataSource("ReportsInvoice_Invoice_Prev_Comment", dsFilteredInvoicesComp))
        'Else
        '    lr.DataSources.Add(New ReportDataSource("dsConcept_Invoice_Prev_GRP_Concept", dsFilteredInvoicesCompConcept))
        'End If
        ...
        Dim pdateFrom As ReportParameter = IIf(ctrlCheckIn.HasFromValue, New ReportParameter("dateFrom", ctrlCheckIn.DateFrom), New ReportParameter("dateFrom"))
        Dim pdateTo As ReportParameter = IIf(ctrlCheckIn.HasToValue, New ReportParameter("dateTo", ctrlCheckIn.DateTo), New ReportParameter("dateTo"))
        Dim p As ReportParameter() = {pdateFrom, pdateTo, puser, photel, pfilter, pHotelName}
        lr.SetParameters(p)
    ...
    Protected Sub DetailReportNew()
        Dim tStart = Now
        Me.LogReport(LogApp.stateStart, "Detail Report rendering started")
        ...
        lr.DataSources.Clear()
        lr.DataSources.Add(New Microsoft.Reporting.WebForms.ReportDataSource("ReportsInvoice_InvoiceJournal_Detail", dsFilteredInvoicesDet))
        FilterString()
        ...
        Try
            ...
            'Response.Close()
            Response.End()
            Me.LogReport(LogApp.stateComplete, String.Format("Detail Report rendering finished in {0} sec.", DateDiff(DateInterval.Second, tStart, Now)))
        Catch ex As Exception
            Me.LogReport(LogApp.stateError, String.Format("Detail Report rendering error: {0}", ex.Message))
            msgPop.ShowError(ex)
        End Try
    End Sub
