...
Partial Class Reports_Logs_InvoiceLog
    ...
    Protected Sub dsLog_ObjectCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceEventArgs) Handles dsLog.ObjectCreated
        Dim ta As LogReportsTableAdapters.LogRep_InvoicesTableAdapter
        ta = e.ObjectInstance
    ...
    Protected Function CheckParams() As Boolean
        ...
        Dim pInvType As New ReportFilterClass(ddlInvType.SelectedItem.Text, ReportFilterClass.ValueType.valString)
        Dim pInvSource As New ReportFilterClass(ddlInvSource.SelectedItem.Text, ReportFilterClass.ValueType.valString)
        ...
        paramLog = "Hotel: " & pHotel.ValueStr & "; " & _
                   ...
                   "InvType: " & pInvType.ValueStr & "; " & _
                   "InvSource: " & pInvSource.ValueStr & "; " & _
    ...
    Protected Sub RenderReport()
        ...
        lr.ReportPath = "Reports\Logs\InvoiceLog.rdlc"

        lr.DataSources.Clear()
        lr.DataSources.Add(New Microsoft.Reporting.WebForms.ReportDataSource("LogReports_LogRep_Invoices", dsLog))
        ...
        Try
            ...
            If cbExcel.Checked Then
                ...
                Response.AppendHeader("Content-Disposition", "attachment; filename=""" + "Invoices Log.xls" + """")
            Else
                bytes = lr.Render("PDF", deviceInfo, mimeType, encoding, extension, streamids, warnings)
                Response.ContentType = "application/pdf"
                Response.AppendHeader("Content-Disposition", "attachment; filename=""" + "Invoices Log.pdf" + """")
            End If
    ...
    Protected Function FilterParam(ByVal ByBooking As Boolean) As String
        ...
        End If
        If txtGroup.Text <> String.Empty Then pfc.Add(String.Format("for group {0}", txtGroup.Text))
    ...
    Protected Sub LogReport(ByVal logState As String, ByVal logText As String)
        Dim l As New LogApp
        l.LogUserAction(My.User.Name, LogApp.objRpt, "Invoices Log", logState, logText)
    End Sub
End Class
