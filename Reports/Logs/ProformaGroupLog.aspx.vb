...
Partial Class Reports_Logs_ProformaGroupLog
    ...
    Protected Sub dsLog_ObjectCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceEventArgs) Handles dsLog.ObjectCreated
        Dim ta As LogReportsTableAdapters.LogRep_ProformaGroupTableAdapter
        ta = e.ObjectInstance
    ...
    Protected Function CheckParams() As Boolean
        ...
        Dim pGroup As New ReportFilterClass(txtGroup.Text, ReportFilterClass.ValueType.valString)
        ...
        '    End If
        'End If
        If Not (pTourOp.HasValue) Or rCkIn > 1 Or rInvNum > 100 Then
            If pTourOp.HasValue Or pTourOpGroup.HasValue Then
                If rCkIn > 1 Or rInvNum > 100 Then
                    errText = "Try set shorter periods or number range."
                End If
            ElseIf pTourOp.HasValue Then
                If rCkIn > 1 Or rInvNum > 100 Then
                    errText = "Try set shorter periods or number range."
                End If
            ElseIf pTourOpGroup.HasValue Then
                If rCkIn > 1 Or rInvNum > 100 Then
                    errText = "Try set specific Tour Operator or shorter periods or number range."
                End If
            Else
                errText = "Try set specific Tour Operator or shorter periods or number range."
            End If
    ...
    Protected Sub RenderReport()
        ...
        lr.ReportPath = "Reports\Logs\ProformaGroupLog.rdlc"

        lr.DataSources.Clear()
        lr.DataSources.Add(New Microsoft.Reporting.WebForms.ReportDataSource("LogReports_LogRep_ProformaGroup", dsLog))
        ...
        Try
            ...
            If cbExcel.Checked Then
                ...
                Response.AppendHeader("Content-Disposition", "attachment; filename=""" + "Group Proformas Log.xls" + """")
            Else
                bytes = lr.Render("PDF", deviceInfo, mimeType, encoding, extension, streamids, warnings)
                Response.ContentType = "application/pdf"
                Response.AppendHeader("Content-Disposition", "attachment; filename=""" + "Group Proformas Log.pdf" + """")
            End If
    ...
    Protected Function FilterParam(ByVal ByBooking As Boolean) As String
        ...
        End If
        If txtGroup.Text <> String.Empty Then pfc.Add("for group like: " & txtGroup.Text)
    ...
    Protected Sub LogReport(ByVal logState As String, ByVal logText As String)
        Dim l As New LogApp
        l.LogUserAction(My.User.Name, LogApp.objRpt, "Group Proformas Log", logState, logText)
    End Sub
End Class
