...
Partial Class Contracts_Special_Application
    ...
    Protected Sub btnAttachAll_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If IsFreeRate() Then
            popShow(False, "Error Special", New Exception("Can't attach free-rate special to contract!"))
        Else
            For Each grv As GridViewRow In gvContracts.Rows
                Dim hrb As Boolean = CType(grv.Cells(1).Controls(1), CheckBox).Checked
                If (hrb) Then
                    Try
                        cid = Int32.Parse(CType(grv.FindControl("hfContID"), HiddenField).Value)
